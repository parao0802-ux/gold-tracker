...
## ✅ 결정 기록 — 평가 기준은 한국금거래소 유지 (KRX 전환 검토 후 철회, 2026-08-06)

한때 **한국거래소(KRX) 금시장 시세로 바꾸려다 사용자가 철회**하고 현행(한국금거래소 '팔 때') 유지로 결론냈다.
**이건 남은 과제가 아니다.** 아래는 같은 조사를 반복하지 않기 위한 참고 자료일 뿐, 착수할 일이 아니다.

철회 이유: KRX 무료 경로(공공데이터포털)가 **하루 1회 갱신(전일 종가)**이라 바꾸면 **평가액이 장중 내내 고정**된다. 지금 쓰는 한국금거래소는 하루 2~4회 고시돼 그래도 움직인다.

### 왜 값이 다른가 (참고용, 다시 조사하지 말 것)
`한국금거래소`(koreagoldx.co.kr, 민간 딜러) ≠ `한국거래소`(KRX, 공식 거래소). 2026-08-06 실측 1g 기준:

| 출처 | 1g | 국제 대비 |
|---|---|---|
| 국제 현물 | ₩195,143 | — |
| KRX 금시장 (전환 검토했다가 철회) | ₩194,280 | −0.4% |
| 한국금거래소 팔 때 ← **현재 앱이 쓰는 값** | ₩189,600 | −2.4% (딜러 매입 마진) |
| 한국금거래소 살 때 | ₩228,533 | +17% (세공비·부가세) |

### KRX 데이터 경로 (나중에 누가 다시 요구하면 참고)
...
- **유일한 경로 = 공공데이터포털** [금융위원회_일반상품시세정보](https://www.data.go.kr/data/15094805/openapi.do). 무료·인증키 필요·**하루 1회 갱신**. 키는 대화나 코드에 넣지 말고 Vercel 환경변수(`DATA_GO_KR_KEY`)로만 다룰 것
- ⚠️ `data.go.kr`은 **개발 환경에서 차단됨**(ECONNREFUSED). 검증하려면 배포된 icn1 함수로 해야 함
...
- 자택 PC에는 Node.js v24 / npm 11이 있음(2026-09-20 확인) → `npx serve`로 정적 서버를 띄워 테스트 가능. 다만 `vercel` CLI는 설치돼 있지 않아 **로컬 서버리스 함수 실행은 여전히 안 됨** (`api/` 검증은 배포본에서)
...
- `api/domestic-gold.js`를 고쳐도 **로컬에서는 검증할 수 없다** — 로컬 접속 시 `API_BASE`가 배포본(`PROD_ORIGIN`)을 가리키므로(`app.js`의 `API_BASE` 분기) 화면에 뜨는 국내 시세는 배포본 함수가 준 값이다. 함수 수정분은 배포해야 확인된다
